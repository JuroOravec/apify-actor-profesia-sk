...
const createMockClientDataset = (overrides?: ClientDataset): ClientDataset => ({
...
const createMockClientRequestQueue = (overrides?: ClientRequestQueue): ClientRequestQueue => ({
...
const createMockKeyValueStoreClient = ({
  log,
...
const createMockRequestQueueClient = ({
  log,
  onBatchAddRequests,
}: {
  log?: (args: any) => void;
...
const createMockDatasetCollectionClient = ({
  log,
...
export const createMockStorageDataset = (
  ...
  return Promise.resolve({
    pushData: async (data) => {
      log?.(`Called MockStorageDataset.pushData (instance ${datasetIdOrName}) with ${JSON.stringify(origArgs)}`); // prettier-ignore
    },
  } as Dataset);
};
